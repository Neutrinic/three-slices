...
A natural conjecture is that $\alpha$-reflection positivity on the bridge form, combined with $\delta$-invariance, \emph{automatically} implies $\theta$-reflection positivity (and hence the Lorentzian continuation). We explain in \S\ref{sec:obstruction} why this fails on any fixed domain: the $\theta$-RP kernel decomposes under $\delta$ into positive and negative parts, and $\delta$-odd states violate $\theta$-RP. The abstract transfer fails precisely on the $(-1)$-eigenspace of the implementing unitary $U$.

For $n=2$ and $n=3$, this obstruction is overcome by \emph{analytic} methods specific to each case (tube domain properties for QFT, conformal structure for CFT), not by abstract algebra. Holomorphic extension to the complex Olshanski semigroup yields a contractivity bound $|K_\theta| \leq K_\alpha$ (\Cref{thm:contractivity}) that is strictly stronger than what the algebraic obstruction alone gives, but still falls short of positivity. The identification of this obstruction, together with the contractivity bound, sharpens the question of simultaneous reflection positivity from a vague open problem to a precise analytic condition (\Cref{conj:simultaneous_RP}).
...
\begin{itemize}
\item $\alpha = \Ad(I_{2,2})$ acts as the Cartan involution on each $\ssl(2,\C)$ factor independently (fixing $\su(2) \oplus \su(2) = \sso(4)$);
...
\end{enumerate}
Then $\omega$ is also $\theta$-reflection positive with respect to $S_\theta = H_\theta \exp(C_\theta)$, yielding simultaneous unitary representations of $\SO(2n)$ and $\SO_0(1,2n{-}1)$.
\end{conjecture}

\begin{remark}[Status and relation to the $\delta$-evenness approach]\label{rem:conj_discussion}
The conjecture is verified for $n=2$ \cite{Author_SplitWedge} and $n=3$ (classical CFT). Conditions (i)--(ii) alone are insufficient (\Cref{prop:failure}). The role of condition (iii) is to replace the indefinite unitary insertion $U$ (eigenvalues $\pm 1$) with the positive modular operator $\Delta^{1/2}$ (spectrum in $\R_{>0}$), thereby overcoming the sign obstruction on $\delta$-odd states.
...
\cite{BW75,BW76} (derived from the spectrum condition, 
Lorentz covariance, and locality); for $n=3$, it follows from 
the Brunetti--Guido--Longo theory of modular localisation 
\cite{BGL02}. For general $n$, the recent work of Neeb, 
\'Olafsson, and collaborators on wedge domains and standard 
subspaces in NCC symmetric spaces \cite{NO23wedge,MNO23} 
provides the natural framework.

The modular-theoretic approach and the $\delta$-evenness approach 
of \cite{Author_CauchySzego} resolve the obstruction by different 
mechanisms. The $\delta$-evenness route shows the $R$-odd/\,$\delta$-odd 
sector is \emph{empty} for scalar fields (and positive 
$\delta$-structure fields generally), making the obstruction 
vacuous. The modular-theoretic route replaces the indefinite 
algebraic intertwiner with the positive modular operator, making 
the obstruction irrelevant even when $\delta$-odd $K$-types are 
present (as for $J_\tau = -\mathrm{id}$ fields). For Wightman 
theories with $J_\tau = +\mathrm{id}$, both mechanisms apply and 
...
The bridge form is $\SO_0(2,2n{-}2)$ for all $n \geq 2$: always exactly two timelike directions. This is forced by \Cref{prop:bridge_unique}. Among real forms $\SO_0(p,q)$, only $p=2$ is Hermitian for $n \geq 3$.
...
\begin{alignat}{2}
\sigma_E(X) &= -\bar{X}^T &\qquad& \text{(compact form: fixes $\su(2n)$)}, \\
\sigma_B(X) &= -I_{n,n}\, \bar{X}^T\, I_{n,n} &\qquad& \text{(bridge form: fixes $\su(n,n)$)}, \\
\sigma_L(X) &= J_n\, \bar{X}\, J_n^{-1} &\qquad& \text{(Lorentzian form: fixes $\su^*(2n)$)},
\end{alignat}
...
\emph{Established:} The algebraic structure---$V_4$, bridge uniqueness, cone preservation---and the compact continuation, for all $n \geq 2$. The obstruction to abstract transfer and the contractivity bound $|K_\theta| \leq K_\alpha$. The $A$-series extension. The full simultaneous reconstruction for $n=2$ (QFT) and $n=3$ (CFT).

\emph{Not established here:} Simultaneous reconstruction for general $n$. The abstract transfer fails (\S\ref{sec:obstruction}). The resolution requires showing that the $\delta$-odd sector is absent from the physical Hilbert space. This is established in the companion paper \cite{Author_CauchySzego}, which proves that all scalar Hardy-space $K$-types on the Type~IV domain are $\delta$-even (a consequence of the rank-2 lattice structure), and that the vector-valued sign is controlled by the fiber intertwiner $J_\tau$. For $J_\tau = +\mathrm{id}$ (bosonic fields and Dirac fermions in even dimensions), the obstruction identified in \S\ref{sec:obstruction} is thereby shown to be vacuous on physical states.

\emph{The relationship to companion papers:} The present paper provides the Lie-algebraic skeleton that identifies the $\delta$-odd obstruction as the sole obstacle to simultaneous reconstruction. The companion paper \cite{Author_CauchySzego} resolves this obstruction via Hardy-space analysis. Together, they supply the input for \cite{Author_SplitWedge}, whose bridge decomposition $\omega(F^{*_\theta} \cdot F) = \omega(F_+^{*_{\Theta_S}} \cdot F_+) - \omega(F_-^{*_{\Theta_S}} \cdot F_-)$ becomes positive-definite once the $\delta$-odd sector is eliminated. The logical dependence is: obstruction (here) $\to$ resolution (\cite{Author_CauchySzego}) $\to$ reconstruction (\cite{Author_SplitWedge}).
