...
reflection positivity across all real forms. This establishes 
that reflection-positive bosonic QFTs on any one real form of 
$\C^d$ determine, via an explicit integral transform, 
reflection-positive theories on every other real form.
\end{abstract}
...
\end{enumerate}
The arithmetic core of this theorem---the identity $k + |\lambda| = 2m_1$, the eigenvalue $(-1)^{k+|\lambda|} = +1$, and the absence of $\delta$-odd $K$-types---has been formally verified in Lean~4 with zero \texttt{sorry} axioms \cite{LeanVerification}.
...
$H^2(D^N_{IV})$. By the theorem of Schmid \cite{Schmid1969} 
and Hecht--Schmid \cite{HechtSchmid1983}, the $K$-type decomposition is:
\begin{equation}
\label{eq:hds-decomp}
...
by the lowest weight condition) and $\lambda$ ranges over certain 
$O(N)$-representations determined by the branching rules.
...
We have established that bosonic quantum field theories on different 
real forms of complexified spacetime are not merely analogous or 
related by formal manipulation---they are \emph{projections} of a 
single holomorphic object, and the projection operator (the 
Cauchy--Szeg\H{o} kernel) is an explicit integral transform. The 
three formulations (Euclidean, Lorentzian, split) are three ``views'' 
of the same entity, connected by continuation through the interior 
of the Type~IV domain.
...
A key lesson of the present analysis is that the $\delta$-odd 
obstruction identified in \cite{Bridge} was an artifact of the 
algebraic method's failure to impose the holomorphic extension 
condition, not an intrinsic feature of the physics. In the scalar 
sector, $\delta$-odd scalar $K$-types do not exist in the 
$K$-type lattice at all---not merely in $H^2(\check{S})$, but 
in $L^2(\check{S})$ as well, since $k + |\lambda| = 2m_1$ is 
even for all integer $m_1$ (Theorem~\ref{thm:disjointness}, 
Appendix~\ref{app:parity}). In the vector-valued sector, 
the $\delta$-eigenvalue is controlled entirely by the fiber 
intertwiner $J_\tau$ (Lemma~\ref{lem:vv-delta-eigenvalue}), and 
for physical bosonic fields ($J_\tau = +\mathrm{id}$) the Hardy 
space is uniformly $\delta$-even. (See Remark~\ref{rem:distributional} 
for the respective roles of the Cauchy--Szeg\H{o} kernel and 
the Bargmann--Hall--Wightman theorem in this result.)
...
universal. Only the inner product formulation varies with the 
field content.
...
\begin{enumerate}
  ...
  \item \textbf{Algebraic obstruction} (\cite{Bridge}): For $d > 4$, 
    the algebraic transfer encounters sign reversals. The obstruction 
    is \emph{precisely characterised} and is the \emph{only} obstacle 
    to a purely algebraic proof.
  \item \textbf{Analytic resolution} (present paper): The 
    Cauchy--Szeg\H{o} kernel provides an analytic transfer that 
    bypasses the obstruction. The ``$\delta$-odd modes'' of the 
    algebraic approach are revealed to be an artifact: in the scalar 
    sector they do not exist; in the vector sector they are controlled 
    by the fiber intertwiner $J_\tau$.
\end{enumerate}

Together, these establish (for bosonic fields and Dirac fermions 
in even dimensions) that reflection-positive quantum field theories 
on any real form of $\C^d$ are in canonical bijection, mediated by 
the Cauchy--Szeg\H{o} kernel of the Type~IV bounded symmetric domain.
...
Cauchy--Szeg\H{o} module: 55 theorems and examples verified with zero
\texttt{sorry} axioms, including the central $\delta$-evenness theorem
$k + |\lambda| = 2m_1$ (\texttt{delta\_evenness}), the eigenvalue identity
$(-1)^{k+|\lambda|} = +1$ (\texttt{delta\_eigenvalue\_positive}), absence
of $\delta$-odd $K$-types (\texttt{no\_delta\_odd\_Ktypes}), Jordan
algebra structure, and Type~IV domain geometry.
