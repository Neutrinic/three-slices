...
\begin{remark}[Scope of the formal verification]
\label{rmk:trinity-scope}
The equivalence cycle in Corollary~\ref{cor:trinity} relies on standard functional-analytic results for two of its four legs: $(1) \Rightarrow (2)$ uses Wick rotation and the forward direction of OS reconstruction, while $(1) \Rightarrow (3)$ uses analytic continuation through the extended tube $\TT'$. These are not verified in Lean. The novel contribution verified in the formalization is the algebraic infrastructure that enables the remaining legs---the sum-kernel rewrite (Step~1), the semigroup structure (Step~5), the tube inclusion $\TT_S \subset \TT'$ (Lemma~\ref{lemma:split-tube}), and the Bridge Lemma factorization $\theta = R \circ \Theta_S$---which are the new content specific to split signature.
\end{remark}
...
The GNS construction yields a Hilbert space $\HH$, a cyclic vector $\Omega$, and a $*$-representation $\pi$ of $\BB(\WW^+)$ with $\omega(F) = \langle \Omega, \pi(F) \Omega \rangle$. (This is a standard application of the GNS theorem for $*$-algebras; the subsequent semigroup extension in Step~3 uses Nelson's theorem and Stone's theorem. These functional-analytic steps are not verified in Lean, which covers only the algebraic infrastructure feeding into them---see the Formal Verification subsection.)
...
\end{equation}
\footnotetext{This identity requires $a$ to be a timelike translation: $\Theta_S$ negates the $(u,v)$ components but preserves $(x,y)$, so the cancellation $\Theta_S(\xi' + a) = \Theta_S \xi' - a$ fails for spatial components. Formally verified: \texttt{thetaS\_shift} in \cite{LeanVerification} proves $\Theta_S(\xi' + a) = \Theta_S \xi' - a$ for $a = (a_u, a_v, 0, 0)$.}
we obtain:
\begin{align}
...
\emph{Step 3: Holomorphic interpolation.} The spectrum condition (S5) implies that $W_2$ is the boundary value of a function $W_2^{\mathrm{hol}}$ holomorphic on the split tube $\TT_S = \R^4 + iV^+_S$. For $t \in [0,1]$, define:
\begin{equation}
    K_t(\xi, \eta) := W_2^{\mathrm{hol}}(\Theta_S \xi - \eta - 2ta + i\epsilon y)
\end{equation}
for small $\epsilon > 0$ and $y \in (V^+_S)^*$, where $(V^+_S)^*$ is the dual cone defined in \eqref{eq:dual-cone}. Since $(V^+_S)^* \subset V^+_S$, the imaginary part $\epsilon y$ lies in $V^+_S$ and the holomorphic function $W_2^{\mathrm{hol}}$ is well-defined at this argument. This interpolates between $K_0 = K$ and $K_1 = K_a$ as $\epsilon \to 0$.
...
Taking $\epsilon \to 0^+$: the integrand $|g(p)|^2 e^{-\epsilon p \cdot y}$ is dominated by $|g(p)|^2$ (which is $\mu$-integrable since $Q_{0,0}[f] = \|\Psi_f\|^2 < \infty$). By dominated convergence:
\begin{equation}
...
By Lemmas \ref{lemma:contractivity-n} and \ref{lemma:symmetric}, $\{T(a_u, 0, 0, 0)\}_{a_u \geq 0}$ is a strongly continuous symmetric contraction semigroup. Strong continuity follows from continuity of $\omega$ tested against translates of Schwartz functions: $\lim_{a \to 0} \|T(a)\Psi - \Psi\|^2 = \lim_{a \to 0} \omega((F - \alpha_a F)^* \cdot (F - \alpha_a F)) = 0$. By the Hille--Yosida theorem, the generator $H_u$ is self-adjoint and non-negative. Similarly for $H_v$. The semigroup property $T(a)T(b) = T(a+b)$ is immediate from $\alpha_a \circ \alpha_b = \alpha_{a+b}$.
...
\end{equation}
In split coordinates, $\theta(u, v, x, y) = (-v, -u, x, y)$, which negates $\tau = (u+v)/\sqrt{2}$ and preserves $\sigma = (u-v)/\sqrt{2}$.\footnote{Formally verified: \texttt{thetaE\_explicit}, \texttt{thetaE\_euclidean\_tau}, and \texttt{thetaE\_euclidean\_sigma} in \cite{LeanVerification}.}
...
\begin{proof}
Let $\alpha_R$ denote the automorphism of $\BB(\WW^+)$ induced by $R$. Since $R$ preserves $\WW^+$ (swapping $u \leftrightarrow v$, both positive\footnote{Formally verified: \texttt{swapR\_preserves\_wedge} in \cite{LeanVerification}.}), $\alpha_R$ maps $\BB(\WW^+)$ to itself.

The $\theta$-involution relates to the $\Theta_S$-involution by $\theta = R \circ \Theta_S$\footnote{The factorization $\theta(p) = R(\Theta_S(p))$ and the conjugation identity $\theta(R(p)) = \Theta_S(p)$ have been formally verified: \texttt{bridge\_factorization} and \texttt{bridge\_conjugation\_identity} in \cite{LeanVerification}.}, giving
\begin{equation}
...
\emph{Geometric coverage.} For any point $(\tau_0, \sigma_0, x, y)$ with $\tau_0 > 0$, the translated wedge $\{(\tau, \sigma) : \tau > |\sigma - \sigma_0|\}$ contains this point (take the shift $s = \sigma_0$). Hence\footnote{Formally verified: \texttt{geometric\_coverage} in \cite{LeanVerification}. In split coordinates, for any $(u,v)$ with $u + v > 0$, the shift $s = (v-u)/2$ gives $u + s = v - s = (u+v)/2 > 0$.}
\begin{equation}
...
The proof uses three involutions: (i) $\Theta_S$, codimension-2, for which positivity is assumed; (ii) $R$, the discrete symmetry converting codimension-2 to codimension-1; (iii) $\theta = R \circ \Theta_S$, codimension-1, whose $c$-dual gives $\mathrm{ISO}(1,3)$. These three involutions form a Klein four-group $V_4 = \{e, \Theta_S, R, \theta\}$: each squares to the identity, and the composition of any two gives the third. All three preserve the split quadratic form $p^2_S$.\footnote{The full Klein four-group structure, the involution property $\Theta_S^2 = R^2 = \theta^2 = \mathrm{id}$, and isometry of all three maps have been formally verified: \texttt{klein\_four\_identity}, \texttt{thetaS\_involution}, \texttt{swapR\_involution}, \texttt{thetaE\_involution}, \texttt{thetaS\_preserves\_norm}, \texttt{swapR\_preserves\_norm}, \texttt{thetaE\_preserves\_norm} in \cite{LeanVerification}.}
\end{remark}
...
The algebraic core of this paper has been formally verified in the Lean~4 proof assistant using the Mathlib library \cite{LeanVerification}. The formalization covers:
\begin{enumerate}[label=(\roman*)]
\item All definitions: split spacetime $\R^{2,2}$, the split metric $g_S$, the three involutions $\Theta_S$, $R$, $\theta$, the split wedge $\WW^+$, the forward cone $V^+_S$, and the dual cone $(V^+_S)^*$.
\item The Klein four-group structure: $\Theta_S^2 = R^2 = \theta^2 = \mathrm{id}$, $\theta(R(p)) = \Theta_S(p)$, and all three involutions preserve $p^2_S$.
\item The Bridge Lemma factorization: $\theta = R \circ \Theta_S$ and the conjugation identity $\theta(R(p)) = \Theta_S(p)$ that enables positivity transfer.
\item Wedge geometry: $R$ preserves $\WW^+$, $\Theta_S$ maps $\WW^+ \to \WW^-$, and $\bigcup_s U_\sigma(s)\WW^+ = \EE^+$.
\item Dual cone structure: $(V^+_S)^* \subset \overline{V^+_S}$, $(V^+_S)^* \neq \varnothing$, $V^+_S$ is not self-dual (explicit counterexample), and $g_S(p,y) \geq 0$ for $p \in \overline{V^+_S}$, $y \in (V^+_S)^*$.
\item The three antiholomorphic involutions $\sigma_L$, $\sigma_E$, $\sigma_S$ on $\C^4$: involution proofs ($\sigma^2 = \mathrm{id}$), composition laws, fixed-point set characterization (real/imaginary part conditions), and signature verification that the complexified quadratic form restricts to $(1,3)$, $(0,4)$, $(2,2)$ on the respective fixed-point sets.
\end{enumerate}
The functional analysis arguments (GNS construction, BCR theorem, Nelson's theorem, Stone's theorem, OS reconstruction) invoke standard results not yet available in Mathlib and are therefore not machine-verified. The Two-Point Trinity (Corollary~\ref{cor:trinity}) is stated as a formal equivalence, but its proof legs rely on the functional analysis results listed above; the Lean formalization verifies the algebraic infrastructure (sum-kernel rewrite, tube inclusion, semigroup structure) but not the equivalence itself. A successful build of the Lean project constitutes a machine-checked proof certificate for the algebraic content listed above; the source code is available at \cite{LeanVerification}.

\emph{Corrections from formalization.} The Lean verification uncovered that the shift identity $\Theta_S(\xi' + a) = \Theta_S \xi' - a$ (equation~\eqref{eq:theta-shift}) requires restriction to timelike translations $a = (a_u, a_v, 0, 0)$, since $\Theta_S$ preserves the spatial components $(x, y)$. This does not affect the proof of Theorem~\ref{thm:main-theorem}, which uses only timelike shifts $a \in \R_+^2 \times \{0\}$.
...
Split Wedge module: 93 theorems verified including Klein four-group structure,
involution identities, dual cone geometry (non-self-duality counterexample
$g_S = -29$), geometric coverage $\bigcup_s W^+(s) = E^+$, tube inclusion
$V_L^+ \to V_S^+$, and the Bridge Lemma factorization
$\theta_E = R \circ \Theta_S$.
The formalization uncovered that the shift identity
$\Theta_S(\xi' + a) = \Theta_S \xi' - a$ requires restriction to timelike
translations $a = (a_u, a_v, 0, 0)$; see footnote to equation~\eqref{eq:theta-shift}.
