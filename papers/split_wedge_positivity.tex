...
\emph{Corrections from formalization.} The Lean verification uncovered that the shift identity $\Theta_S(\xi' + a) = \Theta_S \xi' - a$ (equation~\eqref{eq:theta-shift}) requires restriction to timelike translations $a = (a_u, a_v, 0, 0)$, since $\Theta_S$ preserves the spatial components $(x, y)$. This does not affect the proof of Theorem~\ref{thm:main}, which uses only timelike shifts $a \in \R_+^2 \times \{0\}$.
